defmodule WspomWeb.Live.Weather.Main do
  ...
  when ticks != nil do
    ...
    <%= if @subchart.xticks? do %>
      <%= for tick <- @xticks do %>
        <!-- X ticks -->
        <line :if={tick.text_up == nil} x1={tick.pos} x2={tick.pos} y1={@subchart.graph_pos + @subchart.graph_height} y2={@subchart.graph_pos + @subchart.graph_height + 20} style="stroke:grey;stroke-width:1" />
        <line :if={tick.text_up != nil} x1={tick.pos} x2={tick.pos} y1={@subchart.graph_pos + @subchart.graph_height} y2={@subchart.graph_pos + @subchart.graph_height + 5} style="stroke:grey;stroke-width:1" />

        <!-- X tick labels -->
        <text :if={tick.text_up} x={tick.pos + 4} y={@subchart.graph_pos + @subchart.graph_height + 20} fill="gray" font-size="16" text-anchor="middle">
          <%= tick.text_up %>
        </text>
        <text :if={tick.text_down} x={tick.pos + 4} y={@subchart.graph_pos + @subchart.graph_height + 35} fill="gray" font-size="16" text-anchor="middle">
          <%= tick.text_down %>
        </text>
      <% end %>
    <% end %>
